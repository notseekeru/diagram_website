...
const toPosInt = (v: unknown, fallback: number): number => Math.max(Number.parseInt(String(v ?? ""), 10) || fallback, 0);
...
const sErr = (res: Response, req: Request, status: number, msg: string) => res.status(status).json({ error: msg, requestId: (req as Request & { id: string }).id });
