...
```bash
curl -X POST http://localhost:5000/api/save-diagram \
...
```bash
curl http://localhost:5000/api/get-diagram/DIAGRAM_ID \
  -H "X-API-Key: YOUR_KEY_HERE"
```
...
```bash
curl -X PUT http://localhost:5000/api/diagrams/DIAGRAM_ID \
  -H "Content-Type: application/json" \
  -H "X-API-Key: YOUR_KEY_HERE" \
...
```bash
curl -X DELETE http://localhost:5000/api/diagrams/DIAGRAM_ID \
  -H "X-API-Key: YOUR_KEY_HERE"
```
